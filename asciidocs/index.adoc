= Feuerlöscherdistributionsverwaltungsprogramm
Moritz Eder, Abdulrahman Al Sabagh, Angerer Mona
1.0.0, 2021/22
ifndef::imagesdir[:imagesdir: images]
//:toc-placement!:  // prevents the generation of the doc at this position, so it can be printed afterwards
:sourcedir: ../src/main/java
:icons: font
:sectnums:    // Nummerierung der Überschriften / section numbering
:toc: left

//Need this blank line after ifdef, don't know why...
ifdef::backend-html5[]

// print the toc here (not at the default position)
//toc::[]

== Projektbezeichnung

Feuerlöscherdistributionsverwaltungsprogramm

== Projektauftraggeber

Thomas Stütz

== Projekthintergrund

In jedem Gebäude gibt es Feuerlöscher, die regelmäßig gewartet bzw. ausgetauscht werden müssen. Es bedarf also einer Übersicht, auf der festgehalten werden muss wann welcher Feuerlöscher gewartet werden muss, dass letzte mal gewartet wurde, etc. .

Ein einheitliches Programm auf dem man einen Überblick über alle Räume und Feuerlöscher eines Gebäudes hat würde dies erheblich erleichtern.
////
Wir HTL Schüler müssen oft Diagramme für verschiedene Übungen oder Programme zeichen. Wir machen das entweder auf Papier oder wir verwenden PlantUML zum Erstellen der Diagramme. Das ist relativ mühselig und der Generator würde uns die Arbeit erleichtern.
////

== Projektendergebnis

Ein funktionierendes Programm dass Überblick über alle Räume und Feuerlöscher eines Gebäudes bietet und Wartungstermine zu den jeweiligen Feuerlöschern anzeigt.

////
Ein vollständiger Diagrammgenerator wird bis spätestens Ende des Schuljahres fertig sein. Es wird also ein Projekt zu Forward oder Reverse Engineering. Kriterien für das Ergebnis könnten sein:

. Funktioniert das Generieren?
. Wurde der Programmcode effizient geschrieben?
. Ist die Ausgabe gut leserlich gestaltet?
. Ist es benutzerfreundlich?
////

== Projektziel

Ziele für unser fertiges Projekt sind:

* benutzerfreundliche und übersichtliche Oberfläche
* Feuerlöscher können hinzugefügt und entfernt werden
* die Feuerlöscher werden chronologisch (welcher Feuerlöscher muss als nächstes gewartet werden) dargestellt


////
Das Ziel dieses Projektes wird ein fertiger Diagrammgenerator sein, den Schüler für das Erstellen von Diagrammen nutzen können, damit es nicht mehr so umständlich ist. Es sollte also einfach ein praktischer und schneller Weg sein, Diagramme für Java zu erstellen.
////

== Projektbeschreibung

Mithilfe unseres Projektes kann man einfach und übersichtlich alle Feuerlöscher eines Gebäudes verwalten. Die Feuerlöscher mit ihren dazugehörigen Wartungsterminen werden chronologisch angezeigt. Es können auch einfach Feuerlöscher hinzugefügt oder entfernt werden, falls notwendig.

////
Das Projekt würde circa so aussehen: Es gibt es einen Knopf, der der Auslöser für das Generieren des Diagramms ist. Wenn man also mit einem Programm oder einer Übung in Java fertig ist, klickt man auf diesen Knopf und dann wird ein Diagramm generiert, das so wie ein Diagramm von PlantUML aussehen würde. Das Design wird übersichtlich und lesbar gestaltet sein. Eine Erweiterung für den Generator wäre, dass man bereits fertiggestellte Diagramme wieder in Java Code zurück umwandelt.
////

== Meilensteine

|=======================
|Meilenstein/Ergebnis|Datum
|Plan zur Erstellung eines Prototyps|31.03.2022
|Prototyp Fertigstellung|07.04.2022
|Datenbankverbindung|28.04.2022
|Funktionen, Layout, Tests|05.05.2022 - 12.05.2022
|E-Mail Benachrichtigungen|19.05.2022
|Performanz Optimierung|09.06.2022
|Projekt fertiggestellt|23.06.2022
|=======================

== Projektressourcen

* IntelliJ IDEA
* Github
* Docker

== Projektrisiken

* Falsche Daten beim Austauschen von Feuerlöschern
* Performanzprobleme

== Projektorganisation

Projektleiter: Abdulrahman Al Sabagh: Zuständig für Model, Controller und Deployment

Projektteam +

- Moritz Eder: Zuständig für Unitests +
- Mona Angerer: Zuständig für View und Controller

== ERD-Diagram

[plantuml,erd,png]
----
include::plantuml/erd.puml[]
----

== CLD (im Code)

[plantuml,code-cld,png]
----
include::plantuml/code-cld.puml[]
----

== Anwendungsfalldiagramm

[plantuml,UseCase,png]
----
include::plantuml/ucd.puml[]
----


